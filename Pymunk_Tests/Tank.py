...
class Tank(Car):
    ...
    def create_body_wheels(self):
        ...
        tank_body, shape = self.create_poly(2000, self._track_posx + 140, self._track_posy-80, 460, 40, elasticity=0)
        ...
        body1, shape = self.create_poly(mass, self._track_posx+306, self._track_posy-31, w, h, rot=11*math.pi/16, elasticity=0)
        track_constraint1 = pm.constraints.PivotJoint(body1, track_bodies_top[-1] , (self._track_posx+311, self._track_posy-39))
        body2 , shape = self.create_poly(mass, self._track_posx+295, self._track_posy-18, w, h, rot=12*math.pi/16, elasticity=0)
        track_constraint2 = pm.constraints.PivotJoint(body1, body2, (self._track_posx+302, self._track_posy-24))
        body3, shape = self.create_poly(mass, self._track_posx + 284, self._track_posy - 5, w, h,rot=12 * math.pi / 16, elasticity=0)
        ...
        small_wheel1, shape = self._create_wheel(50, x -20, y - 40, 10, elasticity=0, friction=1)
        small_wheel2, shape = self._create_wheel(50, x + 264, y - 30, 10, elasticity=0, friction=1)
        ...
        const21 = pm.constraints.PinJoint(wheel1, wheel8, (0, 0), (0, 0))
        const22 = pm.constraints.PinJoint(wheel2, wheel8, (0, 0), (0, 0))
        # second from far left
        const23 = pm.constraints.SlideJoint(tank_body, wheel9, (-84, 0), (0, 0), _min, _max)
        const24 = pm.constraints.PinJoint(wheel1, wheel9, (0, 0), (0, 0))
        const25 = pm.constraints.PinJoint(wheel2, wheel9, (0, 0), (0, 0))
